...
export default function Calendar({ onSwitchView }: { onSwitchView?: (view: "list" | "calendar") => void }) {
  ...
  const minDate = new Date(currentYear, currentMonth, todayDay);
  const canBookNextMonth = todayDay >= 25;
  const maxDate = new Date(currentYear, currentMonth + (canBookNextMonth ? 2 : 1), 0);
